...
⚠️ **重要更新 V8.0**: 新增 `auto_memory.py` 强制防呆机制，确保记忆写入100%执行。
...
```bash
python .opencode/skills/shuohan/auto_memory.py pre "计划做什么的简要描述"
```
...
```bash
python .opencode/skills/shuohan/auto_memory.py post "实际完成的简要描述"
```
...
```bash
# 检查是否有未完成的记忆
python .opencode/skills/shuohan/auto_memory.py check
```
...
```bash
# 步骤1: 预写入
python .opencode/skills/shuohan/auto_memory.py pre "用户询问明天廊坊天气"
```
...
```bash
# 步骤3: 完成写入
python .opencode/skills/shuohan/auto_memory.py post "已回复廊坊明天天气：多云、12℃"
```
