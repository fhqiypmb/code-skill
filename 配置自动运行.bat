...
REM     - 选股+盯盘：每工作日 08:50 静默启动（脚本内部等到 09:40 开盘才开扫）
REM     - ML周训练：每周一 08:00
REM ============================================================
...
echo.
echo 将注册以下任务计划：
echo   1) 选股+盯盘  每工作日 08:50 启动（内部等到 09:40 开盘开扫）
echo   2) ML周训练   每周一   08:00 自动运行
echo.
echo 注意：需要电脑在对应时间处于开机状态。
echo.
pause

REM --- 任务1：每工作日 08:50 启动选股+盯盘（内部等 09:40 开扫） ---
...
REM --- 任务2：每周一 08:00 ML训练 ---
schtasks /create /tn "股票_ML周训练" /tr "\"%TRAINBAT%\"" /sc weekly /d MON /st 08:00 /f
if %errorlevel%==0 (echo [OK] 已注册：ML周训练 周一08:00) else (echo [失败] ML训练任务注册失败)
